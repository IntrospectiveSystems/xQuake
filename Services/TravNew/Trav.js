...
//		}
	...
	// Calculte traveltime for a given phase from delta in degrees. z in km
	// Returns null if not phase table exists or parameters out of range
	function traveltime(vlt, phs, delta, z) {
		if(phs in Vlt.Phase) {
	...
	function associate(vlt, delta, z, tobs) {
		let resmin = 1.0e30;
		let objbest;
		let trvbest;
	...
	function GetServices(com, fun) {
		...
		Vlt = this.Vlt;
		com.Services = {};
		com.Services.TravelTime = t;
		com.Services.Associate = assoc;
		if (fun)
			fun(null, com);
