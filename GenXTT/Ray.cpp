...
//------------------------------------------------------------------Init
// Init(): Initialize ray whenever dependencies such as origin change.
void CRay::Setup() {
	int ilay;
...
//	printf("Setup %d dP1:%.6f dP2:%.6f\n", iph, dP1, dP2);
}

//------------------------------------------------------------------setDepth
// Set source depth prior to travel time or delta calculations
void CRay::setDepth(double z) {
	if (pTerra)
		dRad = pTerra->dRadius - z;
	else
		dRad = 6371.0 - z;
}
...
//	printf("Setup dP1:%.6f dP2:%.6f\n", dP1, dP2);
	...
	case RAY_Pdiff:
		...
		trav = t
				+ RAD2DEG * (delta - d) * pTerra->dR[pTerra->iOuter + 1]
						* 111.111 / rcvr / pTerra->dP[pTerra->iOuter + 1];
		*pmin = dP1;
		return trav;
	case RAY_Sdiff:
		rturn = pTerra->dR[pTerra->iOuter + 1];
		...
		trav = t
				+ RAD2DEG * (delta - d) * pTerra->dR[pTerra->iOuter + 1]
						* 111.111 / rcvr / pTerra->dS[pTerra->iOuter + 1];
		*pmin = dP1;
		return trav;
	}
	double p0, p1, p2;
	...
	if (false) {
		double q;
		double qinc = 1.0;
		for (double p = dP1; p < dP2 + qinc; p += qinc) {
			q = p;
			if (q > dP2)
				q = dP2;
			printf("%.4f %.6f\n", q, Fun(q));
...
double CRay::T(double p, double rcvr) {
	double t = Param(FUN_P_TIME, p, rcvr);
	return t;
}
...
//      only significant changes being in the Fun method.
